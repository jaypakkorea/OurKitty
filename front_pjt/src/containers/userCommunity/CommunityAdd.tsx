...
import { CreateCommunity } from 'apis/api/community';
import { UploadOutlined } from '@ant-design/icons';

function CommunityAdd() {
  const navigate = useNavigate();
  const url = window.location.href.split("/");
  const dishId: number = Number(url[url.length - 3])
  ...
  const handleSignupButton = async () => {
    ...
    if (text) {
      setPosting(true)
      try {
        const createCommunite = await CreateCommunity(formData);
        if (createCommunite) {
          navigate(-1);
          setPosting(false)
        }
  ...
  return (
    <Container>
      ...
      <Layout>
        {/* </Form.Item> */}
        <LayoutFlex gap={"8px"} >
          <TagButton onClick={() => setTag('1')} style={tag === '1' ? { background: '#D9D9D9' } : { background: '#F2F2F2' }}
          >찬반토론 </TagButton>
          <TagButton onClick={() => setTag('2')} style={tag === '2' ? { background: '#D9D9D9' } : { background: '#F2F2F2' }}
          >자유 게시판 </TagButton>
          <TagButton onClick={() => setTag('3')} style={tag === '3' ? { background: '#D9D9D9' } : { background: '#F2F2F2' }}
          >고양이 자랑 </TagButton>
        </LayoutFlex>
        <Form>
          <Form.Item
        ...
        <FlexButtonDiv>
          ...
          {!posting ? <OKButton type="submit" onClick={handleSignupButton}>
